...
// تخفیف‌یاب storefronts (see mahem-backend's Store model): a business's own
// branded profile, paid monthly and confirmed manually here after a bank
// transfer — same approve/reject + confirm-payment shape as /dashboard/jobs.
export default async function StoresPage({
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900">فروشگاه‌های تخفیف‌یاب</h1>
        <p className="mt-1 text-sm text-gray-500">
          مدیریت، تایید و تایید پرداخت اشتراک ماهانه فروشگاه‌ها
        </p>
      </div>
        ...
        <table className="w-full text-right text-sm">
          <thead className="border-b border-gray-200 bg-gray-50 text-xs font-semibold text-gray-500">
            <tr>
              ...
              <th className="px-4 py-3">اشتراک تا</th>
          ...
          <tbody>
            {result.items.map(store => (
              <tr key={store.id} className="border-b border-gray-100 last:border-0">
                <td className="px-4 py-3 font-medium text-gray-900">
                  <div className="flex items-center gap-2">
                    {store.logo ? (
                      ...
                      <img
                        src={proxiedImageUrl(store.logo)}
                        alt=""
                ...
                <td className="px-4 py-3 text-gray-600">{formatDate(store.subscriptionExpiresAt)}</td>
                <td className="px-4 py-3">
