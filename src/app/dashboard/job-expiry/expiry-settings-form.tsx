...
import { updateJobExpirySettingAction } from './actions';
...
export function ExpirySettingsForm({ initialExpiryDays }: { initialExpiryDays: number | null }) {
  ...
  const onSave = () => {
    ...
    startTransition(async () => {
      const result = await updateJobExpirySettingAction(formData);
      if (result.success) {
    ...
    <div className="max-w-lg space-y-4 rounded-2xl bg-white p-6 shadow-sm ring-1 ring-gray-200">
      <div>
        ...
        <p className="mt-2 text-xs text-gray-500">
          هر آگهی استخدام جدید، از لحظه ثبت به مدت این تعداد روز فعال می‌ماند و پس از آن
          به‌صورت خودکار آرشیو می‌شود. تمدید (تایید پرداخت دوم به بعد) هم به همین مدت اضافه
          می‌کند. تغییر این عدد فقط روی آگهی‌ها/تاییدهای جدید (بعد از این تغییر) اثر می‌گذارد.
        </p>
      </div>
      ...
      {savedDays != null && !error && (
        <p className="text-sm text-green-600">
          تنظیمات ذخیره شد: آگهی‌های استخدام {savedDays} روز اعتبار دارند.
        </p>
      )}
